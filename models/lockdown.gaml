model lockdown_with_vaccine_logic
...
global {
	...
	float reinfection_chance <- 0.01; // 1% chance to be infected again
	...
	reflex government_intervention when: every(1 #day) {
		...
		ask tested_people {
			...
			if (self.is_infected) {
				...
				// Lockdown district if infection found
				if (self.my_district != nil and !self.my_district.is_locked_down) {
					ask self.my_district {
						self.is_locked_down <- true;
						self.lockdown_start_time <- time;
					}
				}
			} 
			// B. Vaccine Logic: Give to tested people who are NOT infected and limit to 50/day
			else if (vaccine_available and vaccines_given_today < 50 and !self.is_vaccinated) {
				self.is_vaccinated <- true;
				self.is_recovered <- true; // Mark as immune initially
...
species people skills:[moving] {		
	...
	reflex commute {
		bool restricted <- (my_district != nil and my_district.is_locked_down) or is_isolated or is_in_hospital;
		if (restricted) { 
		...
		if (current_date.hour = 8 and target = nil) {
			district work_dist <- district closest_to my_work;
			if (work_dist != nil and !work_dist.is_locked_down) { target <- any_location_in(my_work); }
		}
	...
	reflex spread_virus when: is_infected and !is_recovered {
		...
		ask people at_distance infection_distance {
			if (!self.is_infected) {
				// If vaccinated, they have a 1% chance of being infected again
				float current_proba <- self.is_vaccinated ? (proba_infection * reinfection_chance) : proba_infection;
				if (self.is_recovered and !self.is_vaccinated) { current_proba <- 0.0; } // Natural immunity
